async function getQuestions(topic = "white shark") {
  const responseInfo = await fetch(
    "https://api.openai.com/v1/chat/completions",
    {
      ...
      headers: {
        ...
        Authorization:
          "Bearer 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",
      },
      body: JSON.stringify({
        model: "gpt-3.5-turbo",
        messages: [
          {
            role: "user",
            content: `Write to me about ${topic} using headlines and number and a dot. Write exactly 6 subtopics. Each subtopic must contain at least 200 words. Do not repeat. Subtopics must always end with ':::'. For example:
